...
const TIME_SIGNATURES = {
  '4/4': { top: 4, bottom: 4, units: 8, beatTicks: [0, 2, 4, 6, 8], labels: ['1', '+', '2', '+', '3', '+', '4', '+'] },
  '3/4': { top: 3, bottom: 4, units: 6, beatTicks: [0, 2, 4, 6], labels: ['1', '+', '2', '+', '3', '+'] },
};
...
/* ============================================================
   Level-Definitionen
   Reihenfolge/Umfang exakt gemäß Konzeptnotiz Punkt 4+5: Level 1-5 bauen im
   4/4-Takt schrittweise die Notenwerte aus, Level 6 wechselt zum 3/4-Takt
   (Pflichtumfang laut Notiz Punkt 5). 6/8 bewusst NICHT enthalten - laut
   Notiz nur "optionale spätere Erweiterung", siehe README/CLAUDE.md.
   ============================================================ */

const LEVELS = [
  { title: 'Viertelnoten', timeSignature: '4/4', allowedTypeIds: ['quarter'] },
  { title: 'Viertel + Achtelpaar', timeSignature: '4/4', allowedTypeIds: ['quarter', 'eighth'] },
  { title: '+ Halbe Note', timeSignature: '4/4', allowedTypeIds: ['quarter', 'eighth', 'half'] },
  { title: '+ Pausen', timeSignature: '4/4', allowedTypeIds: ['quarter', 'eighth', 'half', 'quarterRest', 'eighthRest'] },
  { title: 'Gemischt', timeSignature: '4/4', allowedTypeIds: ['whole', 'half', 'quarter', 'eighth', 'wholeRest', 'halfRest', 'quarterRest', 'eighthRest'] },
  // 3/4 fasst max. 6 Achtel-units - "whole" (8) passt nie hinein und bleibt
  // deshalb hier bewusst weg (würde vom Generator ohnehin nie gezogen).
  { title: '3/4-Takt', timeSignature: '3/4', allowedTypeIds: ['half', 'quarter', 'eighth', 'halfRest', 'quarterRest', 'eighthRest'] },
];

const ROUNDS_PER_LEVEL = 4; // Mittelwert aus der in der Notiz vorgeschlagenen Spanne (3-5)
const POINTS_PER_ROUND = 10;
const FIRST_TRY_BONUS = 5;

/* ============================================================
   State + Fortschritt (localStorage)
   Entscheidung (siehe Notiz Punkt 6/8, bewusst statt stillschweigend
   getroffen): Level-Index und Punktestand werden per localStorage
   pro Gerät/Browser gesichert, kein Nutzerkonto/keine Cloud - sonst würde
   jedes Kind bei jedem Tablet-/Browserwechsel wieder bei Level 1 starten,
   was bei einem Klassensatz herumgehender Tablets sehr frustrierend wäre.
   Der genaue Rundenstand INNERHALB eines Levels wird bewusst NICHT
   gesichert (nur der Level-Index) - beim nächsten Öffnen beginnt man dann
   einfach wieder bei Runde 1 des zuletzt erreichten Levels, das ist
   einfacher zu verstehen als "mitten in Level 3, Runde 2" wiederherzustellen.
   ============================================================ */
...
function loadProgress() {
  try {
    ...
    if (!raw) return { levelIndex: 0, points: 0 };
    const parsed = JSON.parse(raw);
    const levelIndex = Math.max(0, Math.min(LEVELS.length - 1, Number(parsed.levelIndex) || 0));
    const points = Math.max(0, Number(parsed.points) || 0);
    return { levelIndex, points };
  } catch (err) {
    return { levelIndex: 0, points: 0 };
  }
}

function saveProgress() {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ levelIndex: game.levelIndex, points: game.points }));
  } catch (err) {
...
// Füllt den Takt lückenlos von Anfang bis Ende mit zufällig gewählten,
// für dieses Level erlaubten Notenwerten - dieselbe Grund-Idee wie
// randomizeMeasure() im Rhythmus-Generator, aber auf die Level-Auswahl
// eingeschränkt. Jedes Level enthält absichtlich immer auch die kleinste
// erlaubte Dauer als Bestandteil eines Teilers der Takt-Kapazität, damit
// die Schleife nie in einem Rest festhängt, der zu keiner erlaubten Dauer passt.
function generateTargetRhythm(level) {
  ...
  while (position < capacity) {
    ...
    const candidates = level.allowedTypeIds.map(noteType).filter((t) => t.units <= remaining);
    if (candidates.length === 0) break; // sollte laut Level-Definition nie eintreten
    const type = candidates[Math.floor(Math.random() * candidates.length)];
    notes.push({ id: uid('n'), typeId: type.id, startUnit: position });
    position += type.units;
  }
  return notes;
}
...
/* ============================================================
   Rendering: Palette (nur die für das aktuelle Level erlaubten Karten)
   ============================================================ */
...
function updateDragVisuals(clientX, clientY) {
  ...
  preview.innerHTML = singleInnerHtml(noteType(drag.typeId || game.attempt.find((n) => n.id === drag.noteId).typeId).icon, anchorPercent(drag.units));
...
/* ============================================================
   Audio (Web Audio API) - gleiche FM-Glocke wie im Rhythmus-Generator,
   damit beide Apps klanglich zusammengehören.
   ============================================================ */

const PLAYBACK_BPM = 80; // fester Wert bewusst ohne Regler - hier geht es um genaues Hinhören, nicht um Tempo-Einstellungen
...
// harter "Buzzer"/Fehlerton für "falsch" (siehe Notiz Punkt 2/4: keine
// Bestrafung, keine Frustration), sondern ein einzelner, weicher, tiefer Ton.
...
function playTargetRhythm() {
  ...
  const unitSeconds = 60 / PLAYBACK_BPM / 2;
  ...
  game.target.forEach((note) => {
    ...
    if (!type.isRest) {
      scheduleTone(startAt + note.startUnit * unitSeconds, duration * 0.92);
    }
    totalDuration = Math.max(totalDuration, note.startUnit * unitSeconds + duration);
  });
...
function renderProgressHeader() {
  levelLabel.textContent = `Level ${game.levelIndex + 1} von ${LEVELS.length} - ${currentLevel().title}`;
...
function onCheck() {
  ...
  const bonus = game.attemptCount === 1 ? FIRST_TRY_BONUS : 0;
  game.points += POINTS_PER_ROUND + bonus;
  game.roundInLevel += 1;
  saveProgress();
  playSuccessSound();
  ...
  if (game.roundInLevel >= ROUNDS_PER_LEVEL) {
    if (game.levelIndex + 1 < LEVELS.length) {
      showFeedback('correct', `Super gemacht! ${bonus ? `+${POINTS_PER_ROUND + bonus} Punkte (Bonus für den ersten Versuch!)` : `+${POINTS_PER_ROUND} Punkte`} - nächstes Level!`);
      game.levelIndex += 1;
      game.roundInLevel = 0;
      saveProgress();
    } else {
      showFeedback('correct', 'Alle Level geschafft! Du bist ein Rhythmus-Profi! 🎉');
    }
    setTimeout(startRound, 2200);
    return;
  }

  showFeedback('correct', bonus ? `Richtig! +${POINTS_PER_ROUND + bonus} Punkte (Bonus für den ersten Versuch!)` : `Richtig! +${POINTS_PER_ROUND} Punkte`);
...
noteVolumeSlider.addEventListener('input', () => {
  ...
  if (noteMasterGain) noteMasterGain.gain.setTargetAtTime(game.noteVolume, audioCtx.currentTime, 0.01);
});
...
game.levelIndex = restored.levelIndex;
